...
if($result2){
    echo "<script>alert('Student successfully signup')</script>";
}}
}


else if($select== "Lecturer"){
//checking existing customer IDs
$sql1="SELECT * FROM lecturer1 WHERE Registration_number='$regNumber'";
$result1=$conn->query($sql1);
if($result1->num_rows > 0) {
    echo "<script>alert('You are already signup!')</script>";
}else{
//inserting customers
$sql2 = "INSERT INTO `lecturer1`(`Registration_number`,`title`,`Full_Name`,`name_with_initials`,
`gender`,`email`,`contact_number`,`address`,`deparment`,`password`,`confirm_password`) 
        VALUES('$regNumber','$title','$fullName','$nameWithInitials','$gender','$email',
        '$mobile','$address','$department','$password','$confirmPassword')";
...
if($result2){
    echo "<script>alert('Lecturer successfully signup')</script>";
}}
}
}
//closing connection
